...
then
  ...
  then
    ...
    deploy_cluster() {
      ...
      task_def=$(printf "$task_template" $AWS_ACCOUNT_ID "postgres://sbr187:Cl34nN33dl3@sbr-prod-rds.c6qzuvxoniof.us-west-1.rds.amazonaws.com:5432/users_prod" $PRODUCTION_SECRET_KEY)
      echo "$task_def"
      register_definition
      update_service  
